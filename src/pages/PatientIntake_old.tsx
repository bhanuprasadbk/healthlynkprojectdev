...
function PatientIntakeContent({ providerMode = false }: { providerMode?: boolean }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        if (selectedService === 'stedi') {
          const hasStediUrl = serviceConfigs.stedi.apiUrl.trim() !== ''
          const hasStediKey = serviceConfigs.stedi.apiKey.trim() !== ''
          if (!hasStediUrl || !hasStediKey) {
            if (!cancelled) setApiPayers([])
            return
          }
        }
        const rows =
    ...
    ;(async () => {
      try {
        ...
        const providerFirstName =
          typeof provider.first_name === 'string' ? provider.first_name.trim() : ''
        const providerLastName =
          typeof provider.last_name === 'string' ? provider.last_name.trim() : ''
        const npi = typeof provider.npi === 'string' ? provider.npi.trim() : ''
